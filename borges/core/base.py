"""
core.base: core classes for a borges data science project
...
class Theory(framework.ProjectBase, chrisjen.Project):
    """Directs construction and execution of a borges data science project.
    
    Args:
        name (Optional[str]): designates the name that is used for internal 
            referencing throughout borges. For example, if an instance needs 
        ...
        clerk (Union[Clerk, Type[Clerk], pathlib.Path, str]): a Clerk-compatible 
            ...
            throughout borges. Defaults to the default Clerk instance. 
        tag (str): a unique tag name for a borges project. The name is used for 
...
class Idea(framework.ProjectBase, chrisjen.Settings):
    """Loads and stores configuration settings for a borges project.
    ...
    Args:
        ...
        skip (Sequence[str]): names of suffixes to skip when constructing nodes
            for a borges project. Defaults to a list with 'general', 'files',
            'borges', and 'parameters'. 
    ...
    skip: Sequence[str] = dataclasses.field(
        default_factory = lambda: ['general', 'files', 'borges', 'parameters'])

 
@dataclasses.dataclass
class Clerk(framework.ProjectBase, chrisjen.Filer):
    """File and folder management for borges projects.

    Creates and stores dynamic and static file paths, properly formats files
    for import and export, and provides methods for loading and saving borges, 
...
class Process(framework.ProjectBase, chrisjen.Component, abc.ABC):
    """Base class for parts of a borges Workflow.
