...
class Parameters(amos.Dictionary):
    """Creates and stores parameters for a borges component.
    
    Parameters allows parameters to be drawn from several different sources, 
    including those which only become apparent during execution of a borges
    project.
    ...
    If a borges class uses a Parameters instance, the 'finalize' method should
    ...
    Args:
        name (str): designates the name of a class instance that is used for 
            internal referencing throughout borges. For example, if a borges 
            ...
            None. 
        contents (Mapping[str, Any]): keyword parameters for use by a borges
        ...
        runtime (Mapping[str, str]): parameters that can only be determined at
            runtime due to dynamic action of borges. The keys should be the
        ...
        required (Sequence[str]): parameters that must be passed when the 
            'implement' method of a borges class is called.
    ...
    def finalize(self, project: base.Theory, **kwargs) -> None:
        ...
        if self.runtime:
            self.add_runtime(borges =project) 
...
class TheoryProcess(base.Process, abc.ABC):
    ...
    Attributes:
        bases (ClassVar[ProjectBases]): library that stores borges base classes 
    ...
    def execute(self, project: base.Theory, 
        ...
        if self.iterations in ['infinite']:
            while True:
                borges =self.implement(borges =project, **kwargs)
        else:
    ...
    def implement(self, project: base.Theory, 
        ...
        if self.contents not in [None, 'None', 'none']:
            borges =self.contents.execute(borges =project, **parameters)
        return borges
...
class Technique(amos.quirks.Loader, TheoryProcess):
    """Primitive object for executing algorithms in a borges workflow.
    ...
    Args:
        ...
        module (str): name of module where 'contents' is located if 'contents'
            is a string. It can either be a borges or external module, as
...
class Worker(TheoryProcess):
    ...
    Args:
        ...
        iterations (Union[int, str]): number of times the 'implement' method 
            ...
            method will continue indefinitely unless the method stops further 
            iteration. Defaults to 1.
        workflow (Phases.Workflow): a workflow of other borges Processs.
