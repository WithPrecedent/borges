...
class Dataset(amos.quirks.Needy, amos.quirks.Element):
    ...
    Args:
        ...
        datatypes (Optional[Dict[str, str]]): keys are column names and values
            are borges proxy datatypes. Defaults to an empty dictionary.
        prefixes (Optional[Dict[str, str]]): keys are column prefixes and
            values are borges proxy datatypes. Defaults to an empty
            dictionary.
    ...
    def combine_rare(self,
        ...
        Args:
            data (borges.base.Dataset): instance storing a pandas DataFrame.
            columns (Optional[Union[List[str], str]]): column names to be checked.
...
class DataState(object):
    """A single state in data processing for a borges project.
